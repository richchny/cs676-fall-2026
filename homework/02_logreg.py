...
def sigmoid(z):
    ...
    #    np.exp(z) / (1 + np.exp(z))
    # )
    # above version will still trigger an overflow because np.where evaluates both
    # but will still ultimately give the correct answer
    # below version avoids the issue by avoiding the relevant branch only
